...
//!
//! GPU addresses are integers; CPU mappings remain raw pointers. All GPU resources are
//! destroyed explicitly and immediately. Calls are single-threaded: use a windowed device
//! on its winit event-loop thread. The device retains an Rc to its window.
//!
//! # Safety
//! For every unsafe API call, handles must belong to the same live device. Keep heaps,
//! resources, descriptors, and GPU address ranges alive until recorded and executing work
//! has finished. Submit every begun command buffer exactly once in the next batch, with
//! the first begun buffer first (it records pending image transitions). Wait for the final
//! submission, destroy resources, then destroy the device. Mapped memory access and Vulkan
//! synchronization, ranges, alignments, shader interfaces, and other Vulkan preconditions
//! remain the caller's responsibility, as in the C++ API.
...
pub struct CommandBuffer {
    state: *mut Device,
    next: *mut CommandBuffer,
    previous: *mut CommandBuffer,
...
struct TextureInitialization {
    ...
    mip_levels: u32,
    array_layers: u32,
    previous: *mut TextureInitialization,
...
unsafe fn append_texture_initialization(
    ...
    debug_assert!(
        (*initialization).owner.is_null()
            && (*initialization).previous.is_null()
            && (*initialization).next.is_null()
    );
...
struct Swapchain {
    ...
    render_views: [RenderView; MAX_SWAPCHAIN_IMAGES],
    ...
    transition_commands: *mut CommandBuffer,
    acquired: bool,
...
pub struct Device {
    ...
    pending_texture_initializations: TextureInitializationList,
    next_command_context: *mut CommandBuffer,
    command_submit_infos: Vec<vk::CommandBufferSubmitInfo<'static>>,
    command_context_count: usize,
    ...
    swapchain: *mut Swapchain,
    acquired_swapchain: *mut Swapchain,
...
impl Device {
    ...
    unsafe fn create_backing_buffer(
        ...
        let mut result = BackingBuffer::default();
        result.buffer = self.vk().create_buffer(
...
/// swapchain format and must be called from the window's winit event-loop thread.
pub fn create_device(desc: &DeviceDesc) -> Result<*mut Device> {
    unsafe {
        debug_assert!(
        ...
        let mut state = Box::new(Device {
            ...
            next_command_context: ptr::null_mut(),
            ...
            acquired_swapchain: ptr::null_mut(),
        ...
        enabled.unified_image_layouts.unified_image_layouts = selected.unified_image_layouts.into();
        ...
        if presentation {
            state.swapchain = Box::into_raw(Box::new(Swapchain {
                state: &mut *state,
                handle: vk::SwapchainKHR::null(),
                images: [vk::Image::null(); MAX_SWAPCHAIN_IMAGES],
                render_views: [RenderView::default(); MAX_SWAPCHAIN_IMAGES],
                ...
                transition_commands: ptr::null_mut(),
                acquired: false,
                recreate_required: false,
            }));
            recreate_swapchain(state.swapchain)?;
        }
...
impl Drop for Device {
    fn drop(&mut self) {
        unsafe {
            ...
            debug_assert!(self.acquired_swapchain.is_null());
            if self.device.is_some() {
                self.drain_contexts();
                if !self.swapchain.is_null() {
                    retire_swapchain_handle(self.swapchain);
                    drop(Box::from_raw(self.swapchain));
...
impl Device {
    ...
    unsafe fn grow_command_context_pool(&mut self) -> Result<()> {
        ...
        self.command_submit_infos
            .push(vk::CommandBufferSubmitInfo::default());
        self.command_context_count += 1;
        if self.next_command_context.is_null() {
            (*context).next = context;
            (*context).previous = context;
            self.next_command_context = context;
        } else {
            (*context).next = self.next_command_context;
            (*context).previous = (*self.next_command_context).previous;
            (*(*context).previous).next = context;
            (*self.next_command_context).previous = context;
        }
        Ok(())
    }
    ...
    unsafe fn destroy_command_contexts(&mut self) {
        ...
        while !self.next_command_context.is_null() {
            let mut context = Box::from_raw(self.next_command_context);
            self.next_command_context = context.next;
            self.destroy_command_context(&mut context);
        }
        self.command_submit_infos.clear();
        self.command_context_count = 0;
    ...
    unsafe fn reset_command_context(&self, context: *mut CommandBuffer) {
        debug_assert!(
            (*context).state.is_null()
                && (*context).retire_value <= self.completed_command_retirement
        );
        require(
            self.vk()
    ...
    unsafe fn reset_retired_command_contexts(&self) {
        ...
        loop {
            if (*context).retire_value != 0
                && (*context).retire_value <= self.completed_command_retirement
            {
                self.reset_command_context(context);
            }
            context = (*context).next;
            if context == self.next_command_context {
                break;
            }
        }
    }
    unsafe fn acquire_command_context(&mut self) -> *mut CommandBuffer {
        ...
        if !(*context).state.is_null()
            || (*context).retire_value > self.completed_command_retirement
        {
            require_error(self.grow_command_context_pool());
            context = (*self.next_command_context).previous;
        }
        if (*context).retire_value != 0 {
            self.reset_command_context(context);
        }
...
pub unsafe fn destroy_timeline_semaphore(semaphore: *mut TimelineSemaphore) {
    ...
    let semaphore = Box::from_raw(semaphore);
    (*semaphore.state)
        .vk()
...
pub unsafe fn wait_idle(device: *mut Device) {
    debug_assert!(
        !device.is_null()
            ...
            && (*device).acquired_swapchain.is_null()
    );
...
pub fn supports_texture_format(device: &Device, format: Format, usage: TextureUsage) -> bool {
    ...
    match format {
        Format::EacRg if !device.texture_compression_etc2 => return false,
        Format::Astc4x4Srgb | Format::Astc4x4Unorm if !device.caps.texture_compression_astc => {
            return false;
        }
        Format::Bc3Srgb | Format::Bc3Unorm | Format::Bc5Rg | Format::Bc7Srgb | Format::Bc7Unorm
...
pub unsafe fn create_gpu_heap(
    ...
    let backing = state.create_backing_buffer(size, usage, required, preferred, avoided)?;
    let gpu = align_up(backing.address, allocation_alignment);
    let cpu = if backing.mapped.is_null() {
        ptr::null_mut()
    } else {
        backing
            .mapped
            .cast::<u8>()
            .add((gpu - backing.address) as usize)
    };
    Ok(GpuHeap {
        range: GpuCpuRange {
            cpu,
        ...
        owner: Box::into_raw(Box::new(GpuHeapOwner {
            state: device,
            backing,
        })),
    })
...
pub unsafe fn destroy_gpu_heap(heap: GpuHeap) {
    ...
    let device = (*owner.state).vk();
    if !owner.backing.mapped.is_null() {
        device.unmap_memory(owner.backing.memory);
    }
    device.destroy_buffer(owner.backing.buffer, None);
    device.free_memory(owner.backing.memory, None);
}
...
pub unsafe fn create_texture_heap(device: *mut Device, byte_count: u64) -> Result<TextureHeap> {
    ...
    let memory = (*device).vk().allocate_memory(
        &vk::MemoryAllocateInfo::default()
            ...
            .memory_type_index((*device).texture_memory_type),
        None,
    )?;
    Ok(TextureHeap {
        size: byte_count,
        owner: Box::into_raw(Box::new(TextureHeapOwner {
            state: device,
            memory,
        })),
    })
...
pub unsafe fn get_texture_size_align(device: *mut Device, desc: &TextureDesc) -> SizeAlign {
    ...
    let requirements = image_memory_requirements(&*device, texture.info());
    SizeAlign {
...
pub unsafe fn create_texture(
    ...
    debug_assert!(!device.is_null() && !heap.owner.is_null() && (*heap.owner).state == device);
    debug_assert_eq!((*device).active_command_buffers, 0);
    let mut prepared = PreparedTexture::new(&*device, desc);
    let image = (*device).vk().create_image(prepared.info(), None)?;
    if let Err(error) = (*device)
        .vk()
        .bind_image_memory(image, (*heap.owner).memory, offset)
    {
        (*device).vk().destroy_image(image, None);
        return Err(error.into());
    }
    let result = Box::into_raw(Box::new(Texture {
        state: device,
        image,
        width: desc.extent.x,
        height: desc.extent.y,
    ...
    append_texture_initialization(
        &raw mut (*device).pending_texture_initializations,
        &raw mut (*result).initialization,
    );
    Ok(result)
}
pub unsafe fn destroy_texture(texture: *mut Texture) {
    ...
    if !(*texture).initialization.owner.is_null() {
        remove_texture_initialization(&raw mut (*texture).initialization);
    }
...
pub unsafe fn create_render_view(
    texture: *mut Texture,
    desc: &RenderViewDesc,
) -> Result<*mut RenderView> {
    debug_assert!(!texture.is_null());
    let texture = &*texture;
    ...
    let view = (*texture.state).vk().create_image_view(&info, None)?;
    Ok(Box::into_raw(Box::new(RenderView {
        state: texture.state,
        view,
...
pub unsafe fn destroy_render_view(view: *mut RenderView) {
    ...
    debug_assert!(!(*view).swapchain_view);
...
pub unsafe fn write_texture_descriptor(
    ...
    debug_assert!(!device.is_null() && !texture.is_null());
    let device = &*device;
    let texture = &*texture;
    ...
    let destination = vk::HostAddressRangeEXT {
        ...
        size: device.heap_properties.image_descriptor_size as usize,
        ..Default::default()
    };
    require(
        (device.functions().write_resource_descriptors)(
            device.vk().handle(),
            1,
            &descriptor,
            &destination,
...
pub unsafe fn write_sampler_descriptor(
    ...
    let destination = vk::HostAddressRangeEXT {
        address: cpu_destination,
        size: device.heap_properties.sampler_descriptor_size as usize,
        ..Default::default()
    };
    require(
        (device.functions().write_sampler_descriptors)(
            device.vk().handle(),
            1,
            &sampler,
            &destination,
...
unsafe fn retire_swapchain_handle(swapchain: *mut Swapchain) {
    ...
    for index in 0..retired.view_count {
        retired.views[index] = (*swapchain).render_views[index].view;
        (*swapchain).render_views[index] = RenderView::default();
...
unsafe fn recreate_swapchain(swapchain: *mut Swapchain) -> Result<()> {
    ...
    for (index, &image) in images.iter().enumerate() {
        ...
        (*swapchain).render_views[index] = RenderView {
            state: device,
            view: views[index],
            width: extent.width,
...
pub unsafe fn acquire(device: *mut Device) -> Option<SwapchainFrame> {
    ...
    loop {
        ...
        (*swapchain).recreate_required =
            suboptimal && swapchain_surface_configuration_changed(swapchain);
        (*device).acquired_swapchain = swapchain;
        return Some(SwapchainFrame {
            render_view: &raw mut (*swapchain).render_views[image_index as usize],
            extent: Uint32x2 {
                x: (*swapchain).width,
                y: (*swapchain).height,
            },
...
unsafe fn create_raster_pso(
    ...
    debug_assert!(!device.is_null() && color_targets.len() <= MAX_COLOR_ATTACHMENTS);
...
pub unsafe fn create_compute_pso(device: *mut Device, compute_spirv: &[u32]) -> Result<*mut PSO> {
    ...
    let info = vk::ComputePipelineCreateInfo::default()
        ...
        .base_pipeline_index(-1);
    let mut pipeline = vk::Pipeline::null();
    let result = ((*device).vk().fp_v1_0().create_compute_pipelines)(
        (*device).vk().handle(),
        vk::PipelineCache::null(),
        1,
        &info,
...
pub unsafe fn begin_commands(device: *mut Device) -> *mut CommandBuffer {
    debug_assert!(!device.is_null());
    let result = (*device).acquire_command_context();
    require((*device).vk().begin_command_buffer(
    ...
    if (*result).timestamp_pool != vk::QueryPool::null() {
        (*device).vk().cmd_reset_query_pool(
            (*result).command_buffer,
            (*result).timestamp_pool,
            0,
            (*device).timestamp_query_count,
        );
    }
    let mut barriers = [vk::ImageMemoryBarrier2::default(); IMAGE_BARRIER_BATCH_SIZE];
    let mut count = 0;
    while !(*device).pending_texture_initializations.first.is_null() {
        let initialization = (*device).pending_texture_initializations.first;
        ...
        if count == IMAGE_BARRIER_BATCH_SIZE {
            record_image_barriers(&*device, (*result).command_buffer, &barriers);
            count = 0;
        }
    }
    if count != 0 {
        record_image_barriers(&*device, (*result).command_buffer, &barriers[..count]);
    }
    let swapchain = (*device).acquired_swapchain;
    if !swapchain.is_null() && (*swapchain).transition_commands.is_null() {
        ...
        record_image_barriers(&*device, (*result).command_buffer, &[barrier]);
...
unsafe fn submit_commands(
    ...
    debug_assert!(!completion.semaphore.is_null() && (*completion.semaphore).state == device);
    debug_assert!(!commands.is_empty() && (*device).active_command_buffers == commands.len());
    debug_assert!(
        (*device).pending_texture_initializations.first.is_null()
            && (*device).pending_texture_initializations.last.is_null()
    );
    for (index, &current) in commands.iter().enumerate() {
        debug_assert!(!current.is_null() && (*current).state == device);
        ...
        (*current).state = ptr::null_mut();
        (*device).active_command_buffers -= 1;
        (&mut (*device).command_submit_infos)[index] = vk::CommandBufferSubmitInfo::default()
...
pub unsafe fn submit(commands: &[*mut CommandBuffer], completion: TimelinePoint) {
    ...
    let device = (*commands[0]).state;
    debug_assert!((*device).acquired_swapchain.is_null());
    debug_assert!(
        commands
            .iter()
            .all(|&command| (*command).swapchain.is_null())
    );
    submit_commands(
        commands,
        device,
...
pub unsafe fn submit_and_present(
    ...
    debug_assert!(!device.is_null() && !(*device).swapchain.is_null() && !commands.is_empty());
    let swapchain = (*device).swapchain;
    debug_assert!(
        (*swapchain).acquired
            && (*commands[0]).state == device
            && (*commands[0]).swapchain == swapchain
            && (*swapchain).transition_commands == commands[0]
    );
    debug_assert!(
        commands[1..]
            .iter()
    ...
    record_image_barriers(
        &*device,
        (**commands.last().unwrap()).command_buffer,
        &[barrier],
    );
    debug_assert!(!context.present_pending && context.swapchain == vk::SwapchainKHR::null());
    require((*device).vk().reset_fences(&[context.presented]));
    (*swapchain).transition_commands = ptr::null_mut();
    submit_commands(
        commands,
        device,
        completion,
    ...
    let result = (*device)
        .swapchains()
        .queue_present((*device).queue, &present);
    ...
    (*device).acquired_swapchain = ptr::null_mut();
...
pub unsafe fn write_timestamp(commands: *mut CommandBuffer, gpu_destination: u64, stage: Stage) {
    ...
    debug_assert!((*commands).timestamp_count < device.timestamp_query_count);
    (&mut (*commands).timestamp_destinations)[(*commands).timestamp_count as usize] =
        gpu_destination;
    device.vk().cmd_write_timestamp2(
        (*commands).command_buffer,
        stage.vk(),
...
pub unsafe fn set_texture_descriptor_heap(commands: *mut CommandBuffer, heap: GpuRange) {
    debug_assert!(!commands.is_null() && !(*commands).state.is_null());
    let device = &*(*commands).state;
    let properties = &device.heap_properties;
...
pub unsafe fn set_sampler_descriptor_heap(commands: *mut CommandBuffer, heap: GpuRange) {
    ...
    let info = make_heap_bind_info(
        heap,
        device.heap_properties.sampler_descriptor_alignment,
        device.heap_properties.min_sampler_heap_reserved_range,
    );
...
pub unsafe fn set_depth_stencil(commands: *mut CommandBuffer, state: &DepthStencilState) {
    ...
    device.cmd_set_depth_test_enable(command, state.depth_test);
    if state.depth_test {
        device.cmd_set_depth_write_enable(command, state.depth_write);
        device
            .cmd_set_depth_compare_op(command, vk::CompareOp::from_raw(state.depth_compare as i32));
    }
    device.cmd_set_stencil_test_enable(command, state.stencil_test);
...
pub unsafe fn begin_render_pass(commands: *mut CommandBuffer, desc: &RenderingDesc<'_>) {
    ...
    let area_view = if !desc.colors.is_empty() {
        desc.colors[0].render_view
    } else if !desc.depth.render_view.is_null() {
        desc.depth.render_view
    } else {
        desc.stencil.render_view
    };
    debug_assert!(!area_view.is_null());
    ...
    for (index, attachment) in desc.colors.iter().enumerate() {
        debug_assert!(!attachment.render_view.is_null());
        colors[index] = vk::RenderingAttachmentInfo::default()
            .image_view((*attachment.render_view).view)
    ...
    let depth = vk::RenderingAttachmentInfo::default()
        .image_view(if desc.depth.render_view.is_null() {
            vk::ImageView::null()
        } else {
            (*desc.depth.render_view).view
        })
        .image_layout(vk::ImageLayout::GENERAL)
    ...
    let stencil = vk::RenderingAttachmentInfo::default()
        .image_view(if desc.stencil.render_view.is_null() {
            vk::ImageView::null()
        } else {
            (*desc.stencil.render_view).view
        })
        .image_layout(vk::ImageLayout::GENERAL)
    ...
    if !desc.depth.render_view.is_null() {
        rendering = rendering.depth_attachment(&depth);
    }
    if !desc.stencil.render_view.is_null() {
...
unsafe fn emit_root_data(commands: *mut CommandBuffer, root: &[u8]) {
    ...
    debug_assert!(!(*commands).state.is_null());
...
pub unsafe fn draw_indexed(
    ...
    (device.functions().cmd_bind_index_buffer)((*commands).command_buffer, &bind);
    device.vk().cmd_draw_indexed(
        (*commands).command_buffer,
        index_count,
        instance_count,
        first_index,
...
pub unsafe fn dispatch(commands: *mut CommandBuffer, root: &[u8], group_count: Uint32x3) {
    debug_assert!(!commands.is_null());
    emit_root_data(commands, root);
    (*(*commands).state).vk().cmd_dispatch(
        (*commands).command_buffer,
        group_count.x,
        group_count.y,
        group_count.z,
...
pub unsafe fn draw_meshlets(commands: *mut CommandBuffer, root: &[u8], group_count: Uint32x3) {
    debug_assert!(!commands.is_null() && !(*commands).state.is_null());
    emit_root_data(commands, root);
    ((*(*commands).state).functions().cmd_draw_mesh_tasks)(
        (*commands).command_buffer,
        group_count.x,
        group_count.y,
        group_count.z,
...
pub unsafe fn copy_memory_to_texture(
    ...
    debug_assert!(!commands.is_null() && !(*commands).state.is_null() && !destination.is_null());
...
pub unsafe fn copy_texture_to_memory(
    ...
    debug_assert!(!commands.is_null() && !(*commands).state.is_null() && !source.is_null());
...
pub unsafe fn barrier(
    ...
    (*(*commands).state).vk().cmd_pipeline_barrier2(
        (*commands).command_buffer,
